...
{
    unsigned char i;
    unsigned char h_4bit = 0;
    unsigned char l_4bit = 0;
...
{
    unsigned char i;
    unsigned char h_4bit = (w_data & 0xf0);
    unsigned char l_4bit = ((w_data << 4) & 0xf0);
    ...
    for (i = 0; i < 24; i++)
    {
        ...
        {
            lcd_sda_out = ((h_4bit & 0x80) == 0x80);
            h_4bit = h_4bit << 1;
        }
        else
        {
            lcd_sda_out = ((l_4bit & 0x80) == 0x80);
            l_4bit = l_4bit << 1;
        }
        lcd_clk = 1;
    }
